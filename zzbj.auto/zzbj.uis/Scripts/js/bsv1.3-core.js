...
var ops = null;
$(function () {
    $("#zoomWallpaperGrid").hide();
    Tip();
    $.ajax({
        type: "Post",
        url: "../Home/GetSysModule/",
        cache: false,
        async: true,
        dataType: "html",
        success: function (data) {
            ...
            $.unblockUI();
            //Mananger.WindowFloatMessage("../Home/FloatWindow/");
        }
    });
    clock();
						...
						[{
						    ...
						    func: function () {
						        if (window != window.parent) {
						            window.parent.location.replace("../Admin/LoginOff");
						        }
						        else {
						            window.location.replace("../Admin/LoginOff");
						        }
						    }
						}]
					];
	...
	return me ={
		 ...
		 bindTheme:function(){
			 ...
			 $("a",themsSetting).click(function(){
					...
					var h = $(window).height();
					var w = $(window).width();
					$("#zoomWallpaper").attr("src","../../Content/css/images/bg/"+src+".jpg").width(w).height(h);
...
//导航栏
			...
			"<img src=\"../../Content/ModulesImages/" + data[i].icon + "\" /><span>" + data[i].name + "</span></a>";
			            }
			            _box = "<div id='dock' class='dock'><div class='dock-container'>" + menu + "</div></div>";
			            me.create();
...
//初始化分页箭头
Arrows = function(){
    var arrows_l = "<div id='arrows_l' class='arrows'><img src='../../Content/ModulesImages/arrows_l_3.png' /></div>";
    var arrows_r = "<div id='arrows_r' class='arrows'><img src='../../Content/ModulesImages/arrows_r_1.png' /></div>";
	return arrows = {
		init:function(){
			arrows.create();
			arrows.setPosition();
		...
		changeStyle:function(t,c){
			...
			if(t>1 && t<DATA.menu.length){
				$("#arrows_l img").attr("src","../../Content/css/images/arrows_l_1.png");
				$("#arrows_r img").attr("src","../../Content/css/images/arrows_r_1.png");
			}
			if(t==1){
				$("#arrows_l img").attr("src","../../Content/css/images/arrows_l_3.png");
				$("#arrows_r img").attr("src","../../Content/css/images/arrows_r_1.png");
			}
			if(t==DATA.menu.length){
				$("#arrows_l img").attr("src","../../Content/css/images/arrows_l_1.png");
				$("#arrows_r img").attr("src","../../Content/css/images/arrows_r_3.png");
			}
		}
	}
}();
	...
	var _folder = "<div id=\"folder\"><a href=\"#\">"+
   		"<div id=\"folder_content\">"+
			"<div id=\"min_icon_folder\">"+
            	"<img width=\"32\" height=\"32\" border=\"0\" src=\"../../Content/ModulesImages/min/folder_o.png\" />" +
        	"</div>"+
	...
	return file = {
		...
		bindStyle:function(){
			$("#folder").mouseenter(function(){
				$("#folder").css({"background-image": "url(../../Content/css/images/bg_task_group_t_over.png)"});
			});
			$("#folder").mouseleave(function(){
				$("#folder").css({"background-image": "url(../../Content/css/images/bg_task_group_t_msg.png)"});
			});
...
Task = $.Class({
	...
	create:function(op)	{
        ...
        $("<img src='../../Content/ModulesImages/min/" + op.icon + "'/><div class='taskItemIconState'></div>").appendTo(taskItemIcon); //图片路径---------------------------------
		var taskItemTxt  = $("<div>",{
			"class":"taskItemTxt",
			text : op.title
		});
    ...
    return me = {
        ...
        bindStyle: function () {
            $(".sonfile").mouseenter(function () {
                $(this).css({ "background-image": "url(../../Content/css/images/bg_task_group_t_over.png)" });
            });
            $(".sonfile").mouseleave(function () {
                $(this).css({ "background-image": "url(../../Content/css/images/bg_task_group_t.png)" });
            });
        },
        bindEvent: function (id) {
            $(".sonMenuList").bind("mouseenter", function () {
                $(this).css({ "background-image": "url(../../Content/css/images/appbutton_mouseover_bg3.png)" });
            });
            ...
            $(".sonMenuList").click(function () {
                ...
                var icon = _this.find("img").attr("src").split("/")[4];
                var sonMenu = "[]"
                ...
                var sonfile = "<a id=\"sonfile_a" + id + "\" href=\"#\">" +
					"<div id=\"sonfile_" + id + "\" class=\"sonfile\">" +
						"<div class=\"min_icon_sonfolder\">" +
							"<img width=\"32\" height=\"32\" border=\"0\" src=\"../../Content/ModulesImages/min/" + icon + "\" />" +
						"</div>" +
						"<div class=\"min_font_sonfolder\">" + title + "</div>" +
					"</div></a>";
                if (jsonSonMenu != null && jsonSonMenu.length > 0) {
                    var sonlist = "";
                    var sonlistTwo = "";
                    var properties = "";
                    for (var i = 0; i < jsonSonMenu.length; i++) {
                        var objects = jsonSonMenu[i];
                        properties = "appid=\"" + objects.appid + "\" url=\"" + objects.url + "\"";
                        var showName = objects.name.length > 7 ? objects.name.substr(0, 6) + "..." : objects.name;
                        if (objects.type != 2) {
                            sonlist += "<div id=\"" + objects.appid + "\" " + properties + " class=\"sonMenuList\" title=\"" + objects.name + "\" align=\"center\">" +
							"<a href=\"#\"><div class=\"sonMenuListIocn\">" +
								"<img width=\"64\" height=\"64\" src=\"../../Content/ModulesImages/" + objects.icon + "\" />" +
								"</div>" +
								"<div class=\"sonMenuListTitle\">" + showName + "</div></a>" +
						"</div>";
                        }
                        else {
                            if (objects.type != null) {
                                sonlistTwo += "<div id=\"" + objects.appid + "\" " + properties + " class=\"sonMenuList\" title=\"" + objects.name + "\" align=\"center\">" +
							"<a href=\"#\"><div class=\"sonMenuListIocn\">" +
								"<img width=\"64\" height=\"64\" src=\"../../Content/ModulesImages/" + objects.icon + "\" />" +
								"</div>" +
								"<div class=\"sonMenuListTitle\">" + showName + "</div></a>" +
                    ...
                    var _box = "<div id=\"sonMenu_box" + id + "\" class=\"sonMenuListBox\"><div class=\"sonMenu_line\">" +
						"<span class=\"sonMenu_font\">基础功能</span></div>" + sonlist + "</div>";
                    if (sonlistTwo != "")
                        _box += "<div id=\"sonMenu_box" + (id + 1) + "\" class=\"sonMenuListBox\"><div class=\"sonMenu_line\">" +
						"<span class=\"sonMenu_font\">统计分析</span></div>" + sonlistTwo + "</div>";
                    ...
                    art.dialog.open(url, /** 弹出ART窗体*/
						...
						"id": id,
						title: title,
						width: 1345,//$(window).width() > 1345 //? (1365 * 0.86) : $(window).width(), //设置窗口宽度自动适应width
						height:520, //($(window).height() - 80) > 629 ? 629 : ($(window).height() - 80),
						max: true,
						min: true,
						close: function () {
						    me.closeMinTask(id);
...
appIcon_t1 = appIcon_t0.extend({
    ...
    create: function (fid) {
        ...
        appIcon.append($("<img>", {
            alt: this.app.name,
            src: '../../Content/ModulesImages/' + this.app.icon, //图片路径-------------------------------------------------------------------------------------------
    ...
    bindEvent: function () {//绑定事件
        this.box.click(function (e) {
            ...
            var jsonSonMenu = eval("(" + sonMenu + ")"); //将json格式的字符串转换为json
            Windows.openApp(id, title, url, icon, jsonSonMenu, "auto", "auto");
        });
    }
});
//来至侧边框的图标
appIcon_t2 = appIcon_t0.extend({
    ...
    create: function () {
        ...
        appIcon.append($("<img>", {
            alt: this.sApp.name,
            src: '../../Content/ModulesImages/' + this.sApp.icon,
            "class": "appButton_appIconImg",
...
mouseStyle = function(){//箭头样式
	$("#arrows_l").mouseenter(function (){
		if(thisPage==1){
			...
			$("#arrows_l img").attr("src","../../Content/css/images/arrows_l_2.png");
		}
	});
	$("#arrows_r").mouseenter(function (){
		if(thisPage==DATA.menu.length){
			...
			$("#arrows_r img").attr("src","../../Content/css/images/arrows_r_2.png");
		}
	});
